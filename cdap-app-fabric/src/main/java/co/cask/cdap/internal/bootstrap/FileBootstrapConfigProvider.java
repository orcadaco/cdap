...
/**
 * Reads the bootstrap config from a file. If there was an error reading the file,
 * logs a warning and returns an empty config.
 */
public class FileBootstrapConfigProvider implements BootstrapConfigProvider {
  ...
  public BootstrapConfig getConfig() {
    try {
      ...
      LOG.warn("Bootstrap file {} does not exist. No bootstrap operations will be executed.");
    } catch (JsonParseException e) {
      LOG.warn("Could not parse bootstrap file {}. No bootstrap operations will be executed.", e);
    } catch (IOException e) {
      LOG.warn("Could not read bootstrap file {}. No bootstrap operations will be executed.", e);
    }
